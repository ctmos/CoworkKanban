...
// Lazy-load content from separate GitHub files (contentRef)
var _contentRefCache={};
function _loadContentRefs(entries){entries.forEach(function(e){if(!e.contentRef||_contentRefCache[e.id])return;_contentRefCache[e.id]=true;fetchFromGitHub(e.contentRef).then(function(r){if(!r||!r.content)return;var el=document.querySelector('[data-entry-id="'+e.id+'"]');if(el){el.textContent=r.content;e.content=r.content;}}).catch(function(){});});}
