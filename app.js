...
function deleteEntry(entryId){confirmAction('Eintrag in Papierkorb verschieben?','Der Eintrag wird in den Papierkorb verschoben und kann wiederhergestellt werden.',function(){var patients=getPatients();var pat=patients.find(function(p){return p.id===_currentPatId;});if(!pat)return;function markTrashed(arr){if(!arr)return;arr.forEach(function(e){if(e.id===entryId){e.trashed=true;e.trashedAt=new Date().toISOString();}});}markTrashed(pat.entries);markTrashed(pat.blog);savePatients(patients);renderPatBlog(pat);showPatientDetail(_patCurrentId);});}

function restoreEntry(entryId){var patients=getPatients();var pat=patients.find(function(p){return p.id===_currentPatId;});if(!pat)return;function untrash(arr){if(!arr)return;arr.forEach(function(e){if(e.id===entryId){delete e.trashed;delete e.trashedAt;}});}untrash(pat.entries);untrash(pat.blog);savePatients(patients);renderPatBlog(pat);showPatientDetail(_patCurrentId);}

function permanentDeleteEntry(entryId){confirmAction('Endg\u00fcltig l\u00f6schen?','Dieser Eintrag wird unwiderruflich gel\u00f6scht.',function(){var patients=getPatients();var pat=patients.find(function(p){return p.id===_currentPatId;});if(!pat)return;if(pat.entries)pat.entries=pat.entries.filter(function(e){return e.id!==entryId;});if(pat.blog)pat.blog=pat.blog.filter(function(e){return e.id!==entryId;});savePatients(patients);renderPatBlog(pat);showPatientDetail(_patCurrentId);});}
